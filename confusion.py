...
def do_math_leukall(gt_file, pd_file, title):
    gt = open(gt_file, 'r')
    gt_array = []
    gt_len = 0
    gt_cm = []
    ...
    acc = accuracy_score(y_actu, y_pred)
    print("Accuracy score sklearn: " + str(acc))
    target_names = ['Echinocyte', 'Erythrocyte', 'Lymphocyte', 'Mononuclear cells', 'Neutrophils', 'Platelet']
    print(classification_report(y_actu, y_pred, target_names=target_names))
    precision_score_weighted = precision_score(y_actu, y_pred, average='weighted')
    print("Precision score weighted: " + str(precision_score_weighted))
    precision_score_macro = precision_score(y_actu, y_pred, average='macro')
    ...
    fbeta2_score_none = fbeta_score(y_actu, y_pred, average=None, beta=2)
    print("F2 measure none: " + str(fbeta2_score_none))
    jaccard_score_weighted = jaccard_score(y_actu, y_pred, average='weighted')
    print("Jaccard score weighted: " + str(jaccard_score_weighted))
    jaccard_score_macro = jaccard_score(y_actu, y_pred, average='macro')
    print("Jaccard score macro: " + str(jaccard_score_macro))
    jaccard_score_none = jaccard_score(y_actu, y_pred, average=None)
    print("Jaccard score none: " + str(jaccard_score_none))
    matthews_corrcoef_score = matthews_corrcoef(y_actu, y_pred)
    print("Matthews correlation coefficient: " + str(matthews_corrcoef_score))
    df_confusion = pd.crosstab(y_actu, y_pred, dropna=False)
    df_conf_norm = df_confusion.div(df_confusion.sum(axis=1), axis="index")
  #  plt.figure(figsize = (10,7))
    plt.title(title)
    sns.set(font_scale=1.4) # for label size
    sns.heatmap(df_conf_norm, cmap='coolwarm', annot=True, annot_kws={"size": 16}, xticklabels=target_names, yticklabels=target_names) 
    tick_marks = np.arange(len(df_conf_norm.columns))
    plt.savefig(path + name + '.png', bbox_inches='tight')
    plt.clf()
    name = 'Confusion Matrix ' + title + ' Post bbox matching'
