...
PATH = '/home/as-hunt/machine-code/temp/'
...
def split_img_label(data_train,data_test, data_val, out):
    ...
    for k in tqdm.tqdm(range(len(val_ind))):
        os.system('cp '+data_val[val_ind[k]]+' ./'+ folder_val + '/'  +data_val[val_ind[k]].split('/')[2])
        os.system('cp '+data_val[val_ind[k]].split('.jpg')[0]+'.txt'+'  ./'+ folder_val + '/'  +data_val[val_ind[k]].split('/')[2].split('.jpg')[0]+'.txt')
...
data_train, data_val, labels_train, labels_val = train_test_split(df[0], du.index, test_size=0.25, random_state=1) # 0.25 x 0.8 = 0.2

# Function split 
split_img_label(data_train,data_test,data_val,'/home/as-hunt/machine-code/temp2/')